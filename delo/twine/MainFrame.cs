...
namespace delo  
{
  public class MainFrame :  DevExpress.XtraEditors.XtraForm, IMessageFilter
  {
    ...
    {   PdfDocument doc = new PdfDocument();
        ...
        if (outlen.Length != 0)
        {
            if (Globals.pol_instr == "pol_ins")
            { this.queriesTableAdapter1.insert_file_pol_ins(Globals.id_doc2, outlen, filename, textEdit1.Text, Globals.id_user); }
            else { 
          if(Globals.otvet=="otvet")
           { this.queriesTableAdapter1.insert_file_otv(Globals.id_doc1, outlen, filename, textEdit1.Text, Globals.id_user); }
           else { 
            this.queriesTableAdapter1.insert_file(Globals.id_doc, outlen, filename, textEdit1.Text, Globals.id_user);
            }}
        }
        doc.Close();
